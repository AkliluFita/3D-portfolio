  ...
  return (
    <>
      {sideType === "left" ? (
        ...
        <motion.div
          className="absolute bottom-[1%] flex flex-row gap-8 font-bold text-gray-500 uppercase opacity-70 dark:text-gray-700 font-primary items-center bg-gray-300 dark:bg-gray-900"
          variants={slideText("100%")}
          initial="initial"
          animate="animate"
        >
          {skillIconData.map((item) => (
            <item.icon
              key={item.name}
              className="lg:text-[5rem] mb-8 text-[0rem]  text-[#808080] mt-4"
            />
          ))}
        </motion.div>
      )}
